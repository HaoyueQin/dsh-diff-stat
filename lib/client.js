window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function boostHunkWithContext(hunk, fileLines) {
			const newLines = contentLines(hunk.newText);
			if (newLines.length === 0) return null;
			let at = -1;
			for (let i = 0; i + newLines.length <= fileLines.length; i++) {
				let matches = true;
				for (let j = 0; j < newLines.length; j++) if (fileLines[i + j] !== newLines[j]) {
					matches = false;
					break;
				}
				if (!matches) continue;
		...
		async function boostEditHunks(diffs, path, cwd) {
			let hasArg = false;
			for (const hunk of diffs) if (isArgHunk(hunk)) {
				hasArg = true;
				break;
			}
		...
		function MutationRow({ toolName, block, cwd, openFile, inspect }) {
			...
			const [boosted, setBoosted] = (0, react.useState)(null);
			const rawDiffs = diffBody?.card.diffs ?? null;
			const boostable = open === true && model.state === "ok" && rawDiffs !== null && rawDiffs.some(isArgHunk);
			(0, react.useEffect)(() => {
				...
				let alive = true;
				boostEditHunks(rawDiffs, path, cwd).then((next) => {
		...
		function TurnCard(props) {
			...
			const [boosted, setBoosted] = (0, react.useState)(() => new Map());
			...
			const boostedRef = (0, react.useRef)(boosted);
			...
			(0, react.useEffect)(() => {
				...
				(async () => {
					for (const path of revealed) {
						...
						if (file === void 0 || !file.diffs.some(isArgHunk)) continue;
							...
							children: [
								...
								revealedFile && (0, react_jsx_runtime.jsx)("div", {
									...
									children: (0, react_jsx_runtime.jsx)(DiffWindow, {
										diffs: boosted.get(file.path)?.diffs === file.diffs ? boosted.get(file.path).result : file.diffs,
										maxHeight: 320
									})
								}),
