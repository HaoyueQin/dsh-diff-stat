window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const turnChangesDefinition = {
			...
			update: (context, match) => {
				...
				if (match.event.type === "tool/result") {
					...
					if (result === void 0 || result === null) return context.state;
			...
			buildLocationData: (context, scope) => scope !== "turn" || context.state === void 0 ? null : {
				...
				value: {
					changed: context.state.changed,
					hasCodeDispatch: context.state.hasCodeDispatch
				}
			}
		};
		//#endregion
		...
		function TurnCard({ matched, turn, sessionId, openFile, getCwd, useSession, t }) {
			const [expanded, setExpanded] = (0, react.useState)(false);
			...
			const dispatchFiles = (0, react.useMemo)(() => {
				...
				if (cache !== null && cache.snapshot === snapshot && cache.turn === turn && cache.keys.length === keys.length) {
					let same = true;
					for (let i = 0; i < keys.length; i++) if (cache.keys[i] !== keys[i] || cache.nodes[i] !== snapshot.chat.nodes.get(keys[i])) {
						same = false;
						break;
					}
