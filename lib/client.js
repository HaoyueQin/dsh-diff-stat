window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function diffStats(diffs) {
			...
			let removed = 0;
			for (const hunk of diffs) {
				const newLines = contentLines$1(hunk.newText);
				const oldLines = hunk.oldText === null ? [] : contentLines$1(hunk.oldText);
		...
		function boostHunkWithContext(hunk, fileLines) {
			...
			outer: for (let i = 0; i + newLines.length <= fileLines.length; i++) {
				for (let j = 0; j < newLines.length; j++) if (fileLines[i + j] !== newLines[j]) continue outer;
				at = i;
				break;
			}
			if (at === -1) return null;
		...
		async function boostEditHunks(diffs, path, cwd) {
			...
			const fileLines = contentLines$1(content);
			let changed = false;
			...
			(0, react.useEffect)(() => {
				...
				(async () => {
					...
					if (result === null) setState({
						kind: "error",
						message: t("peek.hostUnavailable")
					});
						...
						kind: "error",
						message: result.error ?? t("peek.readFailed")
					});
					else setState({
						kind: "text",
		...
		function apply(ctx) {
			...
			if (registry !== void 0) registry.register(turnChangesDefinition);
			else {
