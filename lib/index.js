		...
		handler: async (req, res) => {
			...
			try {
				...
				if (action === "files.read") {
					...
					respond(res, 200, {
						kind: "text",
						content: truncated ? utf8SafeSlice(resolved.bytes.subarray(0, READ_CAP)).toString("utf8") : resolved.text,
						truncated,
						size: resolved.bytes.length
					});
					return;
				...
				if (action === "undo") {
					...
					const results = [];
					for (const file of files) results.push(await undoFile(String(body["cwd"] ?? ""), {
