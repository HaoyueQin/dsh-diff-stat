...
const OPEN_BRIDGE_TIMEOUT_MS = 1e4;
...
async function undoFile(cwd, session, turn, file) {
	try {
		...
		if (resolved.bytes.length > UNDO_TEXT_LIMIT) return {
			path: file.path,
			ok: false,
			error: "file too large to undo safely (" + resolved.bytes.length + " bytes)"
		};
		...
		try {
			mode = (await lstat(target.filename)).mode & 511;
		} catch {
			return {
				path: file.path,
				ok: false,
				error: "file changed while being accessed (stat)"
			};
		}
...
function openWith(sessionController, cwd, requestedPath, target) {
		...
		(async () => {
			try {
				...
				if (target === "vscode") {
					...
					const probeExit = await new Promise((resolveProbe) => {
					...
					const child = spawn("code \"" + resolvedTarget.filename + "\"", {
						shell: true,
						detached: true,
						stdio: "ignore"
					});
...
function snapshotKey(cwd, session, turn) {
	return cwd.replace(/[/\\]+$/, "") + "\0" + session + "\0" + turn;
}
...
async function captureSnapshot(cwd, session, turn) {
	if (typeof turn !== "number" || !Number.isInteger(turn) || turn < 1) return captureSnapshotInner(cwd, session, turn);
			...
			try {
				...
				if (action === "files.read") {
					...
					if (target.size > READ_CAP) {
						...
						if (Buffer.from(head.text, "utf8").includes(0)) {
							respond(res, 200, {
								kind: "binary",
								truncated: true,
								size: head.size
				...
				if (action === "undo") {
					...
					if (!Array.isArray(files) || files.length === 0) {
						respond(res, 200, {
							ok: false,
							error: files !== void 0 && Array.isArray(files) && files.length === 0 ? "no files to undo" : "files must be a non-empty array",
							results: []
						});
						return;
					}
