...
[![dsh](https://img.shields.io/badge/dsh-%E2%89%A50.1.5-rc.1-4D6BFE?style=flat-square)](https://github.com/deepseek-ai/deepseek-harness)
...
- **内核目标：harness ≥ 0.1.5-rc.1** — 单一构建只面向该产线：`uiConversation` 事件注册表、`tool.call.toolview` keyed 槽与 `conversation.chat.turnTail` 链均为 `0.1.5-rc.1` 形态，diff hunk 读自工具持久化的 wire `meta`。PTC dispatch 以 `tool/ptc-dispatch(-start)` 到达；更名前记录的 `tool/code-dispatch(-start)` 拼写仍被匹配，使更名之前写入的会话保留其汇总卡。更旧的内核**不在**本版本支持范围——请在这些内核上安装 `dsh-diff-stat@0.1.8`（或更早覆盖它们的旧版本）
- **行内 +N −M 徽标** — 接管 `edit`、`write` 与 `str_replace_editor` 的 stock 变更行（`str_replace_editor` 为按需启用，默认关闭；keyed 低优先阴影，卸载自动还原）。计数是真实变更行数——与 diff 渲染共用同一趟 LCS：运行中按参数预估，结算后取精确值
...
- **徽标与 diff**：注册进 `tool.call.toolview` keyed 槽的 `edit`/`write`/`str_replace_editor` 键，priority −1 阴影 shipped 行（`str_replace_editor` 为按需启用）；行 diff 按权威链提取：工具持久化的 wire meta（含 ±3 行文件上下文），PTC 子调用回退到调用时参数推导——窗口截断丢掉调用头时仍可从 result meta 渲染。轮末聚合器同样统计 meta-only 结算
- **轮末汇总卡**：`ConversationNodeDefinition` 聚合器（`turn/start`、`tool/call`、`tool/result(append)`、`tool/ptc-dispatch`——以及更名前记录的 `tool/code-dispatch` 拼写——及 `-start` 证据）发布 Turn 数据，`conversation.chat.turnTail` 链认领渲染——结构遵循官方 `ui-deliverables` 模式。dispatch 文件从 stock 会话工具树 join：其 `tool-call` 节点已把每个 dispatch 按 rootCallId 折叠进根调用的 `subCalls`。纯计算 `run_code` 轮（无文件变更）仍会认领后渲染空——已接受，select 看不到 join 结果。冷打开的纯 PTC 轮可能缺卡（dispatch 记录无 turn 坐标，best-effort）
- **上下文增强**：参数来源的 hunk 在构造时按对象身份标记；展开时增强器经围栏 API 读取文件（LRU 缓存）、定位片段的 after 形态并以共享行重建 hunk，无法定位的原样渲染
- **host 半**（可选）：同源前缀路由提供围栏 API（files.read、每轮快照 capture、undo、open-with）——realpath 包含性解析前后双查、符号链接拒绝、UTF-8 回环校验、显示读取 512 KiB 上限并带截断标记、undo 32 MiB 门限、原子写。两种打开动作都在围栏校验通过后才执行：资源管理器定位把解析后的路径交给 harness 自带的 `sessionController` remote（`action: 'reveal'`），而 harness 无等价能力的 VS Code 仍走本地 spawn。host 半缺席时相关操作自动隐藏
...
> **内核兼容性：** 本构建只面向 harness `>= 0.1.5-rc.1`。更旧的内核需要
> 旧版本插件 —— 请使用 `dsh-diff-stat@0.1.8`。
